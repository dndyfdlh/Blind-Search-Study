...
def depth_limited_search(graph, selectedCity, limit, selectedEndingCity):

    father[selectedCity] = None
    call_to_depth_limited_search(graph, selectedCity, 1, limit, selectedEndingCity)

def call_to_depth_limited_search(graph, city, level, limit, selectedEndingCity):
    ...
    if foundValue == False:
        for neighbor in graph.get(city):
            if not depths.get(neighbor):
                ...
                children += 1
                call_to_depth_limited_search(graph, neighbor, level + 1, limit, selectedEndingCity)
...
limit = int(input("Tentukan limit pencarian (1 - 23): \n"))

depth_limited_search(citiesRoutes, selectedCity, limit, selectedEndingCity)
